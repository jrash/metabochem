---
title: "Significant Metabolites Analysis - Training Set (Fixed Data)"
author: "Jeremy Ash"
date: "May 22, 2017"
...
write.csv(univariate_res_control, "healthstate_anova_wsig_control_training.txt", row.names = F)
...
write.csv(univariate_res_control, "healthstate_anova_wsig_control_training_block.txt", row.names = F)
...
write.csv(univariate_res_control, "healthstate_anova_wsig_control_training_serum.txt", row.names = F)
...
univariate_res_control$significance <- univariate_res_control$pvalues < .05
sum(univariate_res_control$significance)
write.csv(univariate_res_control, "healthstate_anova_wsig_control_training_plasma.txt", row.names = F)
...
univariate_res_control$significance_raw <- univariate_res_control$pvalues < .05
univariate_res_control$significance_adj <- univariate_res_control$adj_pvalues < .05
write.csv(univariate_res_control, "healthstate_anova_wsig_control_training_serum_nonpara.txt", row.names = F)
...
univariate_res_control$significance_raw <- univariate_res_control$pvalues< .05
univariate_res_control$significance_adj <- univariate_res_control$adj_pvalues < .05
write.csv(univariate_res_control, "healthstate_anova_wsig_control_training_plasma_nonpara.txt", row.names = F)
```


```{r}
sigs <- read.csv("C:/Users/jrash/ubuntu_share/Google Drive/FourchesLab/metabolimics/significant_metabolites/serum/healthstate_anova_wsig_control_test_serum_nonpara.txt")
sum(sigs$significance_raw)
sigs_test <- sigs$variables[(sigs$pvalues < .05) == T]
sigs$variables[(sigs$adj_pvalues < .05) == T]

sigs <- read.csv("C:/Users/jrash/ubuntu_share/Google Drive/FourchesLab/metabolimics/significant_metabolites/serum/healthstate_anova_wsig_control_training_serum_nonpara.txt")
sum(sigs$significance_raw)
sigs_training <- sigs$variables[(sigs$pvalues < .05) == T]

sigs_training[sigs_training %in% sigs_test]

sigs <- read.csv("C:/Users/jrash/ubuntu_share/Google Drive/FourchesLab/metabolimics/significant_metabolites/plasma/healthstate_anova_wsig_control_test_plasma_nonpara.txt")
sum(sigs$significance_raw)
sigs_test <- sigs$variables[(sigs$pvalues < .05) == T]
sigs$variables[(sigs$adj_pvalues < .05) == T]

sigs <- read.csv("C:/Users/jrash/ubuntu_share/Google Drive/FourchesLab/metabolimics/significant_metabolites/plasma/healthstate_anova_wsig_control_training_plasma_nonpara.txt")
sum(sigs$significance_raw)
sigs_training <- sigs$variables[(sigs$pvalues < .05) == T]
...
sigs <- read.csv("C:/Users/jrash/ubuntu_share/Google Drive/FourchesLab/metabolimics/significant_metabolites/both/healthstate_anova_wsig_control_training_block.txt")
sum(sigs$significance_raw)
sigs_training <- sigs$variables[(sigs$pvalues < .05) == T]
...
# There is a problem here, the three large intensity cancer data points are
# called healthy in the paper.  P-Value: 0.95

d_serum$xylose[71]
d_serum$Health_State[71]
row.names(d_serum)[71]
# pulled the raw intensity of this patient with cancer status from the database
# again.  The value is right
log(1337, base = 2)

d_serum$xylose[21]
d_serum$Health_State[21]
row.names(d_serum)[21]
# pulled the raw intensity of this patient with cancer status from the database
# again.  The value is right
log(1172, base = 2)
